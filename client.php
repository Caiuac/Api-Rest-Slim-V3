<?php
	...
	function curl($dados="",$tipo="",$meta="",$GET=false) {
		...
		if ($GET) {
			$curl = curl_init("http://localhost/slimApi01/exemplo?dados=" . $dados);
		} else {
			$curl = curl_init("http://localhost/slimApi01/exemplo");
		}
	...
	if ($_POST) {
		...
		switch ($_POST['metodo']) {
			case 'GET':
				...
				break;
			case 'POST':
				$post = "dados=".$_POST['dados'];
				$retorno = curl($post,"POST",CURLOPT_POST);
				break;
			case 'PUT':
				$put = "dados=".$_POST['dados'];
				$retorno = curl($put,"PUT",CURLOPT_CUSTOMREQUEST);
				break;
			case 'DELETE':
				$del = "dados=".$_POST['dados'];
				$retorno = curl($del,"DELETE",CURLOPT_CUSTOMREQUEST);
				break;
